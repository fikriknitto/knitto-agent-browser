...
export function FileGrid({
  ...
  return (
    <div
      className={cn(
        "relative min-h-0 flex-1 select-none overflow-auto rounded-[10px] border border-black/10 bg-black/[0.02] dark:border-white/8 dark:bg-[rgba(9,12,20,0.55)]",
        dragOver &&
          "border-blue-500/55 shadow-[inset_0_0_0_2px_rgba(59,130,246,0.15)] dark:border-blue-500/55"
      )}
    ...
    >
      {dragOver && (
        <div
          className="pointer-events-none absolute inset-0 z-[2] flex items-center justify-center bg-white/85 text-[0.95rem] font-semibold text-blue-700 dark:bg-slate-900/82 dark:text-blue-300"
          aria-hidden="true"
        >
      ...
      {loading && entries.length === 0 ? (
        <p className="flex min-h-48 flex-col items-center justify-center p-8 text-center text-black-40 dark:text-slate-400">
          Memuat…
        </p>
      ) : empty ? (
        <div className="flex min-h-48 flex-col items-center justify-center p-8 text-center text-black-40 dark:text-slate-400">
          <p>Folder kosong</p>
          <p className="mt-1.5 max-w-md text-[0.82rem] text-black-40 dark:text-slate-500">
