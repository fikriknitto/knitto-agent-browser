...
import { storagePathAffectsPaths } from "@/lib/utils/prompt-attachment";
...
type FileManagerPanelProps = {
  ...
  /** `attach` = picker lampiran; `manage` = browser library (halaman Files). */
  selectionMode?: "attach" | "manage";
  /** Extra class on outer wrapper (e.g. page layout without modal padding). */
  className?: string;
  onSelectEntry: (
    entry: StorageEntry,
...
export function FileManagerPanel({
  ...
  return (
    <div className={cn("flex min-h-0 flex-1 flex-col px-5 pt-1", className)}>
      <div className="flex flex-wrap items-center gap-3 pt-0.5 text-xs text-black-40 dark:text-slate-400">
        <span>
        ...
        {manageEntry && (
          <span className="text-blue-600 dark:text-blue-300/90">
            Dikelola:{" "}
            <span className="font-medium text-blue-700 dark:text-blue-200">{manageEntry.name}</span>
          </span>
        )}
      </div>
