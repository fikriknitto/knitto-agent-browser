...
import { getStoredApiDataToken } from "@/lib/api/api-data-client";
import {
  getAgentRunResults,
  listAgentRuns,
...
import { isAcceptedStorageEntry } from "@/lib/utils/prompt-attachment";
import { buildRunEvidence, type RunEvidenceItem } from "@/lib/utils/run-evidence";
import {
  modalBackdrop,
  modalHeader,
  modalRoot,
      ...
      >
        <header className={modalHeader}>
          ...
          <Button
            variant="ghost"
            size="icon-sm"
            className="border-0 bg-transparent text-black-40 hover:bg-black/5 hover:text-black-100 dark:text-slate-300 dark:hover:bg-slate-600/85 dark:hover:text-slate-50"
            aria-label="Tutup"
            onClick={onClose}
          >
            <XIcon size={16} />
        ...
        <div className="flex gap-1 border-b border-black/10 px-4 pt-1 dark:border-white/8">
          {(
            [
              ["uploaded", "Uploaded"],
              ["from_run", "Dari run"],
            ...
            <button
              ...
              className={cn(
                ...
                tab === id
                  ? "bg-black/5 text-black-100 dark:bg-white/10 dark:text-slate-100"
                  : "text-black-40 hover:text-black-80 dark:text-slate-500 dark:hover:text-slate-300"
              )}
              onClick={() => {
                setTab(id);
                setSelected([]);
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          {tab === "uploaded" ? (
            ...
            <div className="flex min-h-0 flex-1 flex-col md:flex-row">
              <aside className="flex max-h-[40%] w-full shrink-0 flex-col border-b border-black/10 md:max-h-none md:w-[280px] md:border-b-0 md:border-r dark:border-white/8">
                <div className="min-h-0 flex-1 overflow-y-auto">
                  {runsError ? (
                    <p className="p-3 text-xs text-red-600 dark:text-red-400">{runsError}</p>
                  ) : null}
                    ...
                    {runs.map((run) => (
                      <li key={run.runId}>
                        <button
                          type="button"
                          className={cn(
                            "flex w-full flex-col gap-0.5 border-b border-black/5 px-3 py-2 text-left text-xs hover:bg-black/5 dark:border-white/5 dark:hover:bg-white/5",
                            pickedRunId === run.runId && "bg-blue-500/10 dark:bg-blue-500/15"
                          )}
                          onClick={() => void openRunMedia(run.runId)}
                        >
                          <span className="font-semibold text-black-100 dark:text-slate-100">
                            #{run.runId}
                          </span>
                  ...
                  {runsLoading && !runs.length ? (
                    <p className="p-3 text-xs text-black-40 dark:text-slate-500">Memuat runs…</p>
                  ) : null}
                </div>
              </aside>
              <div className="min-h-0 flex-1 overflow-y-auto p-3">
                {!pickedRunId ? (
                  <p className="text-sm text-black-40 dark:text-slate-500">
                    Pilih run untuk melihat evidence.
                  </p>
                ) : null}
                {runMediaLoading ? (
                  <p className="text-sm text-black-40 dark:text-slate-500">Memuat media…</p>
                ) : null}
                {selectError && tab === "from_run" ? (
                  <p className="mb-2 text-xs text-red-600 dark:text-red-400">{selectError}</p>
                ) : null}
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          isSelected
                            ? "border-emerald-500/60 bg-emerald-500/15 dark:bg-emerald-900/30"
                            : "border-black/10 bg-black/[0.03] hover:border-emerald-500/35 dark:border-white/10 dark:bg-black/30"
                        )}
                        onClick={() => toggleRunMedia(entry)}
                      >
                        <span className="line-clamp-2 w-full text-black-100 dark:text-slate-200">
                          {entry.name}
                        </span>
                        <span className="text-[0.6rem] text-black-40 dark:text-slate-500">
                ...
                {pickedRunId && !runMediaLoading && !runMedia.length ? (
                  <p className="text-sm text-black-40 dark:text-slate-500">
                    Tidak ada evidence yang bisa dilampirkan.
                  </p>
                ) : null}
              </div>
            </div>
        ...
        <footer className="flex items-center justify-between gap-4 border-t border-black/10 bg-black/[0.03] px-5 py-3.5 dark:border-white/8 dark:bg-[rgba(9,12,20,0.65)]">
          <span className="text-[0.82rem] text-black-40 dark:text-slate-400">
            {selected.length > 0
              ? `${selected.length} file dipilih`
              : "Belum ada file dipilih"}
            {tab === "uploaded" && selectError ? (
              <span className="ml-2 text-red-600 dark:text-red-400">{selectError}</span>
            ) : null}
          </span>
