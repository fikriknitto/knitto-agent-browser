...
import { Button, Input, Label, Select, Textarea } from "@/components/chat/ui";
...
export function PromptShortcutFormModal({
  ...
  const handleGenerate = async () => {
    ...
    setGenerating(true);
    try {
      const result = await generateShortcut({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const saved = await savePromptShortcut(mode, payload, shortcut?.id);
      onSaved(saved);
      onClose();
    } catch (err) {
